...
4. **Відновити базу — ДО першого запуску.** Найнадійніше — зупинити бота на
   СТАРОМУ сервері (`docker compose down`; це коректно згортає WAL) і
   скопіювати `data/bot.db` напряму:
   `scp ubuntu@СТАРИЙ:~/tg-bot-sho-poisty/data/bot.db data/`
   Якщо старого сервера вже немає — відновіть з нічного бекапу на R2
   (`~/backup-script`, README.md у тій теці).
   Якщо базу не відновити, перший `up -d` створить порожню, і бот
